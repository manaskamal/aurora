...
; 250  : void Stall(uint32_t millis) {

$LN5:
	mov	DWORD PTR [rsp+8], ecx
	sub	rsp, 24

; 251  : 	uint64_t current = apic_timer_count;

	movsxd	rax, DWORD PTR apic_timer_count
	mov	QWORD PTR current$[rsp], rax
$LN2@Stall:

; 252  : 	while (apic_timer_count - current < millis);
...
; 245  : 	return (read_apic_register(LAPIC_REGISTER_ICR) & (1 << 12)) != 0;
...
; 236  : void apic_send_sipi(uint8_t id, uint8_t vector) {
...
; 237  : 	while (read_apic_register(0x30) & (1 << 12))
...
; 239  : 
; 240  : 	write_apic_register(0x31, id << 24);
...
; 241  : 	write_apic_register(0x30, vector | (6 << 8) | (1 << 14));
...
; 228  : void apic_send_init(uint8_t id) {
...
; 229  : 	while (read_apic_register(0x30) & (1 << 12))
...
; 231  : 
; 232  : 	write_apic_register(0x31, id << 24);
...
; 233  : 	write_apic_register(0x30, 5<<8);
...
; 221  : uint64_t icr_dest(uint32_t processor) {
...
; 223  : 		return ((uint64_t)processor << 32);
...
; 225  : 		return ((uint64_t)processor << 56);
...
; 214  : void timer_sleep(uint32_t ms) {

$LN5:
	mov	DWORD PTR [rsp+8], ecx
	sub	rsp, 24

; 215  : 	uint32_t tick = ms + apic_timer_count;
...
; 216  : 	while (tick > apic_timer_count)
...
; 152  : void  apic_timer_interrupt(size_t p, void* param) {
...
; 258  : void initialize_cpu(uint32_t processor) {
...
; 260  : 	uint64_t* address = (uint64_t*)0xA000;

	mov	QWORD PTR address$[rsp], 40960		; 0000a000H

; 261  : 	
; 262  : 	
; 263  : 	uint64_t ap_init_address = (uint64_t)x86_64_ap_init;
...
; 265  : 	
; 266  : 
; 267  : 	uint64_t aligned_address = (uint64_t)address;
...
; 269  : 	
; 270  : 
; 271  : 	uint64_t *old_pml = (uint64_t*)x86_64_get_boot_pml();

	call	x86_64_get_boot_pml
	mov	QWORD PTR old_pml$[rsp], rax

; 272  : 	
; 273  : 	for (int i = 1; i <= processor; i++) {
...
; 274  : 		if (i == 8) //MAX number of processor : 8 on SMP (non-NUMA system)
...
; 277  : 		
; 278  : 		void* stack_address = x86_64_pmmngr_alloc();

	call	x86_64_pmmngr_alloc
	mov	QWORD PTR stack_address$6[rsp], rax

; 279  : 		*(uint64_t*)(aligned_address + 8) = (uint64_t)old_pml;

	mov	rax, QWORD PTR aligned_address$[rsp]
	mov	rcx, QWORD PTR old_pml$[rsp]
	mov	QWORD PTR [rax+8], rcx

; 280  : 		*(uint64_t*)(aligned_address + 16) = (uint64_t)stack_address;

	mov	rax, QWORD PTR aligned_address$[rsp]
	mov	rcx, QWORD PTR stack_address$6[rsp]
	mov	QWORD PTR [rax+16], rcx

; 281  : 		*(uint64_t*)(aligned_address + 24) = ap_init_address;

	mov	rax, QWORD PTR aligned_address$[rsp]
	mov	rcx, QWORD PTR ap_init_address$[rsp]
	mov	QWORD PTR [rax+24], rcx

; 282  : 		*(uint64_t*)(aligned_address + 32) = (uint64_t)x86_64_phys_to_virt((size_t)x86_64_pmmngr_alloc());
...
; 283  : 		void* cpu_struc = (void*)x86_64_phys_to_virt((uint64_t)x86_64_pmmngr_alloc());
...
; 286  : 		*(uint64_t*)(aligned_address + 40) = (uint64_t)cpu_struc;
...
; 288  : 		
; 289  : 
; 290  : 		write_apic_register(LAPIC_REGISTER_ICR, icr_dest(i) | 0x4500);
...
; 292  : 
; 293  : 
; 294  : 		size_t startup_ipi = icr_dest(i) | 0x4600 | ((size_t)address >> 12);
...
; 295  : 		write_apic_register(LAPIC_REGISTER_ICR, startup_ipi);
...
; 299  : 		write_apic_register(LAPIC_REGISTER_ICR, startup_ipi);
...
; 160  : int x86_64_initialize_apic(bool bsp) {
...
; 164  : 		apic_base = (size_t)x86_64_phys_to_virt(0xFEE00000);
...
; 166  : 		apic_base =  x64_read_msr(IA32_APIC_BASE_MSR);
...
; 181  : 		x64_write_msr(IA32_APIC_BASE_MSR, x86_64_virt_to_phys(apic_base));
...
; 183  : 		x64_write_msr(IA32_APIC_BASE_MSR, apic_base);
...
; 184  : 	//! Sends EOI to APIC
; 185  : 	setvect(0xFF, apic_spurious_interrupt);

	lea	rdx, OFFSET FLAT:?apic_spurious_interrupt@@YAX_KPEAX@Z ; apic_spurious_interrupt
	mov	ecx, 255				; 000000ffH
	call	setvect

; 186  : 	write_apic_register(LAPIC_REGISTER_SVR, read_apic_register(LAPIC_REGISTER_SVR) |
; 187  : 		IA32_APIC_SVR_ENABLE | 0xFF);
...
; 188  : 	
; 189  : 	//!Register the time speed
; 190  : 	write_apic_register(LAPIC_REGISTER_TMRDIV, 0xa);  //0xa
...
; 191  : 
; 192  : 	/*! timer initialized*/
; 193  : 	size_t timer_vector = 0x40;

	mov	QWORD PTR timer_vector$[rsp], 64	; 00000040H

; 194  : 	setvect(timer_vector, apic_timer_interrupt);
...
; 195  : 
; 196  : 	size_t timer_reg = (1 << 17) | timer_vector;

	mov	rax, QWORD PTR timer_vector$[rsp]
	bts	rax, 17
	mov	QWORD PTR timer_reg$[rsp], rax

; 197  : 	write_apic_register(LAPIC_REGISTER_LVT_TIMER, timer_reg);
...
; 199  : 	write_apic_register(LAPIC_REGISTER_TMRINITCNT, 76);  //100 , 500
...
; 200  : 
; 201  : 
; 202  : 	x64_outportb(PIC1_DATA, 0xFF);
...
; 204  : 	x64_outportb(PIC2_DATA, 0xFF);
...
; 205  : 
; 206  : 	//! Finally Intialize I/O APIC
; 207  : 	//map_page (ioapic_base,ioapic_base,0);
; 208  : 	if (bsp)
...
; 209  : 		ioapic_init((void*)x86_64_phys_to_virt(0xfec00000));
