...
; File e:\aurora kernel\kernel\string.cpp
_TEXT	SEGMENT
count$ = 0
first$ = 32
second$ = 40
...
; 48   : 
; 49   : 	size_t count;
; 50   : 	for (count = 0; count < length; count++)
...
; 51   : 	{
; 52   : 		if (((unsigned char*)first)[count] != ((unsigned char *)second)[count])
...
; 53   : 		{
; 54   : 			if (((unsigned char*)first)[count] < ((unsigned char *)second)[count])
...
; 64   : 
; 65   : 	return (0); //return successful code
...
; 40   : void memset(void *targ, uint8_t val, uint32_t len){
...
; 136  : char* strchr(char* str, int character) {
...
; 109  : 	unsigned count;
; 110  : 
; 111  : 	if ((destString == (char*)NULL) || (sourceString == (char*)NULL))
...
; 114  : 	}
; 115  : 
; 116  : 	if (maxLength > MAX_STRING_LENGTH)

	cmp	QWORD PTR maxLength$[rsp], 25
	jbe	SHORT $LN6@strncpy

; 117  : 		maxLength = MAX_STRING_LENGTH;

	mov	QWORD PTR maxLength$[rsp], 25
$LN6@strncpy:

; 118  : 
; 119  : 	for (count = 0; count < maxLength; count++)
...
; 120  : 	{
; 121  : 		destString[count] = sourceString[count];
...
; 126  : 
; 127  : 	if (count >= MAX_STRING_LENGTH)

	cmp	DWORD PTR count$[rsp], 25
	jb	SHORT $LN1@strncpy

; 128  : 	{
; 129  : 		return (destString = NULL);
...
; 103  : 
; 104  : 	return (n == 0) ? 0 : (size_t)((unsigned char)*s1 - (unsigned char)*s2);
...
; 72   : 	while (!(res = *(unsigned char*)str1 - *(unsigned char*)str2) && *str2)
